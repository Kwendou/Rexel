<?php
/**
 * This User will have functions that hadles user registration,
...
class Cl_User
{
	...
	private function alias($nom, $prenom, $n) {
		$alias=substr($nom,0,1);
		$alias.=substr($nom,1+$n,1);
		$alias.=substr($prenom,0,2);
		$alias=strtoupper($alias);

		$nbre_alias=0;

    /* Détermine le nombre de lignes du jeu de résultats */
		$query = "SELECT `ALIAS` FROM `utilisateurs` WHERE `ALIAS` LIKE '".$alias."'" ;

		if ($result = $this->_con->query($query)) {

			/* Détermine le nombre de lignes du jeu de résultats */
			$nbre_alias = $result->num_rows;
